	...
	*/
	...
	function get_available_backups($simple = false) {
		...
		usort($files, function($a, $b){
			$upload_dir = wp_upload_dir();
			$dir = $upload_dir['basedir'].self::DS()."nifty-backups".self::DS();
	...
	public function integrity_check($file_list,$file) {
		...
		foreach ($backups_in_progress['zip_chunks'] as $key => $val) {
			if (@file_exists($key)) {
				$this->logthis(__LINE__. " Comparing files in ".$key."","verbose",true);
				$za = new ZipArchive(); 
				$za->open($key); 

				for( $i = 0; $i < $za->numFiles; $i++ ){ 
				    $stat = $za->statIndex( $i ); 
				    $zip_arr_list[rtrim($stat['name'],$this->DS())] = $key;
				}
				$za->close();
			}
		}
		...
		$this->logthis(__LINE__. " Initial files read: ".$files_read,"verbose",true);
		...
		$this->logthis(__LINE__. " Files missed: ".$files_missed,"verbose",false);
		...
		$this->logthis(__LINE__. " Reassessed files read: ".$backups_in_progress['files_read'],"verbose",false);
	            ...
	            else if (is_file($key) === true)
	            {
	            	...
	            	if ($val['nname'] == 'dump.sql') {
	            		...
	            		if ($val < -1) {
	            			...
	            			$this->logthis(__LINE__. " [Attempt $val] Adding from STRING for ".$key,"verbose",true);
